...
module Wire4Client
  ...
  class Billing
    ...
    attr_accessor :url_pdf

    # Url del archivo xml de la factura
    attr_accessor :url_xml
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'emission_at')
        self.emission_at = attributes[:'emission_at']
      end
      ...
      if attributes.has_key?(:'url_pdf')
        self.url_pdf = attributes[:'url_pdf']
      end

      if attributes.has_key?(:'url_xml')
        self.url_xml = attributes[:'url_xml']
      end
    end
    ...
    def valid?
      status_validator = EnumAttributeValidator.new('String', ['PAID', 'OMMITED', 'WAITING_PAYMENT', 'EMISSION_PENDING'])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ['PAID', 'OMMITED', 'WAITING_PAYMENT', 'EMISSION_PENDING'])
    ...
    def hash
      [amount, emission_at, end_date, id, start_date, status, transactions, url_pdf, url_xml].hash
    end
